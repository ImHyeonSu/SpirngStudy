...
//import com.tuyano.springboot.MyData_5_1;
//import org.springframework.data.jpa.repository.JpaRepository; // 새로운 리포지터리 생성을 하기 위한 토대임
//import org.springframework.stereotype.Repository;
//
//
//@Repository
//public interface MyDataRepository_3 extends JpaRepository<MyData_5_1, Long> {
//	public MyData_5_1 findById(long name); //이 메서드는 MyData_5_1 에 구현하지 않았는데 실행된다. 이는 리포지터리의 장점이며 리포지터리는 메스더명을 기준으로 엔터티 검색 처리를 자동 생성하게 돼 있다. 즉 리포지터리는 메서드 선언만 작성하고 구체적인 처리를 구현할 필요가 없다!!
//	public List<MyData_5_1> findByNameLike(String name); // name으로 Like검색을 한다. "%"+str+"%"와 같은 형태로 와일드 카드를 지정할 필요가 있다.
//	public List<MyData_5_1> findByIdIsNotNullOrderByIdDesc(); //모든 요소를 ID가 높은 순으로 가져온다, 내림차순으로 불러오는 기능이 추가되어있다.
//	public List<MyData_5_1> findByAgeGreaterThan(Integer age);//age는 숫자 프로퍼티, 이것은 age의 값이 인수에 지정한 값보다 큰 값을 검색한다.
//	public List<MyData_5_1> findByAgeBetween(Integer age1, Integer age2);//두개이 age값 사이에 있는 것을 가져온다.
//
//}
...
////>> 메서드명은 카멜기법을 따른다 - 각 단어의 첫 글자만 대문자로(최초단어 제외), find[by~~][그 외 검색 조건][OrderBy 등]